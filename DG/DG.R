...
DG_batch <- function(R,head,seed,p,g,gsm,ntrain,ntune,ntest,sigma2,ediu=0.05,Gmode=0,batch=0)
{
  for ( i in 1:R )
  {
    fname = sprintf("%s/data%03d",head,batch+i)
    if ( !file.exists(fname) )
      DG(seed+batch+i,p,g,gsm,ntrain,ntune,ntest,sigma2,ediu=ediu,Gmode=Gmode,savefile=fname)
  }
}

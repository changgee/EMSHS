...
#          NULL if no edge (EMSH)
# a_sigma, b_sigma, a_omega, b_omega: prior params
# descent: "newton" uses the newton direction for optimizing alpha
#          "diagonal" uses only the diagonal entries of the Hessian matrix
...
EMSHS <- function(y,X,mus,nu,E=NULL,a_sigma=1,b_sigma=1,a_omega=2,b_omega=1,sigma_init=NULL,alpha_init=NULL,descent="diagonal",w=1,eps=1e-5)
{
  ...
  # initialize for DWL
  DWL.X <<- as.matrix(X)*sqrt(w)
  DWL.y <<- as.vector(y)*sqrt(w)
